...
def setup_pitzer(solutes):
    ...
    LAMBDA,LAMBDA_inds = make_parameter_matrix(solutes,'LAMDA',property_dict)
...
def loggamma_and_osmotic(carray,T,zarray,
    ...
    dim_tensors = np.array([carray.shape[0],carray.shape[0],carray.shape[0]],dtype=np.intc)
    ...
    valence_prod_1 = -1*zarray[B1_inds[:,0]]*zarray[B1_inds[:,1]] #(1,1),(2,1) < 4, (2,2) = 4, (3,2),(4,2) > 4
    ...
    J_prime = (x_mn*jprime(x_mn) - 0.5*x_mm*jprime(x_mm) - 0.5*x_nn*jprime(x_nn))/(2*I)
