...
def residual_and_jacobian(molals,TK,activity_function,
    ...
    res2 = formula_matrix@molals - formula_vector
    res = np.hstack([res1,res2])
    
    #Jacobian approximation
    mole_fractions = molals/(np.sum(molals))
    activity_hessian_diag = (1-mole_fractions)/molals
    upper_jacobian = -stoich_matrix[:,1:]*activity_hessian_diag
    lower_jacobian = formula_matrix[:,1:]
